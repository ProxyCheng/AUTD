...
# 动画被打断会补发一次 recipe_dropped,该回调必须命中仍有效的旧 target)→ 之后
# 基类 unbind() 才清 target 引用。
...
func _connect_signals():
	var workshop: Workshop = target as Workshop
	if not workshop:
		return
...
func _disconnect_signals():
	...
	# 清空配方列表:移除全部行并置空列表的 target 引用,防行 _process 在目标释放后
	# 触碰已失效 target(freed instance)。重绑后 refresh 会重新 populate。
	# clear() 会终结进行中的拖拽;若落位动画被打断,列表先补发 recipe_dropped 再清空 ——
	# 此刻 target 引用尚未被基类清掉,_on_recipe_dropped 仍能落到旧目标上。
