...
# 手持工具耐久条:值 = 工人手上仓(Labor.hand_bag)里那件工具的耐久占比;
...
# 接受任意 Entity;非 Labor(没有 hand_bag)置空,本条自动隐藏。
...
# 工人手上仓里那件工具;无 / 已 freed / 非 Tool 时返回 null。
func _held_tool() -> Tool:
	if labor == null or not is_instance_valid(labor.hand_bag):
		return null
	var carrier: Object = labor.hand_bag.peek_state()
