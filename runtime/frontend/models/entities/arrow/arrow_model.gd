...
#
# 弹道公式本体在 backend(Ballistic.arc_slope / Ballistic.launch_pitch / Ballistic.GRAVITY),
...
static func _arc_slope(in_t: float, in_launch_height: float, in_flight_time: float) -> float:
	return Ballistic.arc_slope(in_t, in_launch_height, in_flight_time)

# 离弦仰角(弧度):委托 backend,见 Ballistic.launch_pitch。
static func launch_pitch(in_horizontal_distance: float, in_launch_height: float, in_move_speed: float) -> float:
	return Ballistic.launch_pitch(in_horizontal_distance, in_launch_height, in_move_speed)
...
func set_flight(in_progress: float, in_direction: Vector2, in_launch_height: float, in_flight_time: float, in_move_speed: float):
	...
	var arc: float = 0.5 * Ballistic.GRAVITY * in_flight_time * in_flight_time
