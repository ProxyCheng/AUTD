...
# 在 Turret 基类(与前端 TurretModel 一一对应),本类只覆写它的调参钩子,不复制任何状态机代码。
# 与弩炮的区别:
#   * 弹药为 cannonball(车间以石头合成),弹丸为 Cannonball 实体;
#   * 弹丸更慢、装填/蓄力更久、备弹更少,但射程更远,命中造成圆形区域伤害(见 Cannonball)。
# 发射几何沿用 Turret 的共用静态值(两模型同为炮塔式底座,尺度相近);若日后需单独标定,
# 覆写 pivot()/spawn() 即可,无需改基类。

# —— 与弩炮不同的调参(覆写 Turret 钩子)——
...
# 与后端 fire() 共用同一值(与 Turret 的 PIVOT_*/SPAWN_* 同属"前后端共用弹道参数"约定)。
# 取得比弩箭(10)慢得多,是为了让炮弹走抛物线而非直射:离弦仰角由弹速反解(见 Trajectory.aim_pitch_for),
