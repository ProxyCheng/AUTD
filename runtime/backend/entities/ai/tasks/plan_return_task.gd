...
# 顶岗归还·规划叶子:一份活干完(is_work_done 判定)后、走归还点之前,重新决定
# "工具还回容器,还是继续留在手上"。
#
# 为什么必须在这里重算:ManBuildingTask._write_return_plan 在**派活时**就写好了归还计划,
# 而那一刻机器显然还有活干(否则不会派工)——"机器现在还需不需要这件工具"只有在产出之后
# 才可知:本工程一次顶岗只产一件(Workshop._produced_this_shift),最后一件可能正好填满
# 输出仓,机器随即无可行配方(active_recipe == null)。于是决策只能在 work 步骤之后、
# move_return 步骤之前重做一次。
#
# 判定:
