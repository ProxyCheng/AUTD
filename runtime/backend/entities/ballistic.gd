...
# 弹道投射物基类:箭矢/炮弹共用"发射参数 → 抛物线飞行 → 命中"契约。
# 子类只决定命中效果(Arrow 单体伤害 / Cannonball 区域伤害),飞行逻辑不重复实现。
#
# —— 弹道物理(前后端共用同一套公式:发射器瞄准与投射物视觉都读它,避免两份实现)——
...
# —— 弹道表现数据(由发射者在 fire() 里注入;供 frontend 画抛物线视觉)——
# 这些是"从发射口射出"的表现参数:发射高度(离地)、预计飞行时长(命中时刻),
# 以及内部计时,用于把竖直方向抬成抛物线。命中仍由 2D position==target_position 判定,
# 这里只额外暴露给 frontend 用于竖直位移,不改变命中逻辑。
var launch_height: float = 0.0    # 发射瞬间的世界高度(发射口离地)
...
# 精确收敛到 1(Y=0),避免"投射物未落地就被吞"的超前跳变。追踪弹每帧刷新 target_position,
