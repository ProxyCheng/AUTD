...
- **主基地的仓是 `MainBaseBag` 子类**(不是裸 `Bag`):通配(接受任意类型)、无限容量、`preferred_min_count = 0`(永不作为需求方 —— 无限仓一旦被当需求方,会把全图库存吸进自己),`preferred_max_count = 0` 表示"有货即供给"。**落库走分层、基地只作兜底**:`Logistics.find_nearest_bag` 找落库点时先比 `_deposit_rank` —— 同类型仓且 `count_of < preferred_max_count`(2)> 同类型仓未满(1)> 通配仓(0),`deposit_priority` 只在同层内比较。故工人卸货先找"该类型的专仓"(料堆、作坊输入/产出仓…),全图没有专仓收得下才落主基地。取货方向不变:`withdraw_priority = WITHDRAW_FIRST`,工人要工具仍先来基地拿。**注意由此产生的单向性**:只要该类型存在收得下的专仓,基地里这类货就"只出不进"(取它排第一、放它排最后)—— 这正是"专仓优先、基地兜底"的代价,曾经的"双 FIRST 主动枢纽"已废弃。**同一个仓不要混搭 FIRST/LAST** —— 那会组成单向流,实测会把基地抽干。语义细节与理由见 `main_base_bag.gd` 的类注释。
...
- **做完即提交**:功能/修复单元按本节自查项验证通过后**直接提交**,不必等用户发话,更不要停下来问"要不要提交";同一功能单元内的任何后续修正(自查发现、验证失败、用户反馈,或发现某处改动被误并进了别的提交)一律用 `git commit --amend` 并入原提交(维持"一个功能一个提交"),不要新起修补提交。amend 仅限本机尚未 push 的提交;已 push 的不重写历史。
